...
def detect_nudity():
    ...
    # Load the font from a local .ttf file
    font_path = "Inconsolata-Bold.ttf"  # 替换为您的字体文件路径
    font = ImageFont.truetype(font_path, 40)  # 指定字体大小为 40px
    used_positions = []

    def check_overlap(position, width, height):
        for used_x0, used_y0, used_x1, used_y1 in used_positions:
            if (
                used_x0 < position[0] + width
                and used_x1 > position[0]
                and used_y0 < position[1] + height
                and used_y1 > position[1]
            ):
                return True
        return False
                ...
                if label in all_labels:
                    ...
                    while check_overlap(confidence_position, text_width, text_height):
                        confidence_position = (
                            ...
                            confidence_position[1] + text_height + 10,
                        )
                    draw.text(
                        confidence_position, confidence_text, fill=color, font=font
                    )
                    used_positions.append(
                        (
                            confidence_position[0],
                            confidence_position[1],
                            confidence_position[0] + text_width,
                            confidence_position[1] + text_height,
                        )
                    )
